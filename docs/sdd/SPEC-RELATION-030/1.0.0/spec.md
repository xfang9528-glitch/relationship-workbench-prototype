...
depends_on_specs: "SPEC-RELATION-002@1.0.0, SPEC-RELATION-010@1.0.0, SPEC-RELATION-017@1.0.0, SPEC-RELATION-022@1.0.0, SPEC-RELATION-028@1.0.0, SPEC-RELATION-029@1.0.0"
...
- **明确不做**：不新增微信通讯录 roster，不补生产碎银三人设 cohort 或朋友圈 MCP，不把 current allocation 冒充三账号完整，不改 People/朋友圈内容流，不跨生产仓。
...
两张来源卡一眼可横向比较同样六项；每个数字都有明确范围。微信能从本次 canonical 导出拆出的项目显示 exact；没有可信通讯录 roster 的完整好友数显示“受阻”，并可在说明中展示“本批次单聊中出现 N 人”。碎银当前已读片段显示 partial，只有 T028 三账号完整回执成立后才可显示 exact；碎银朋友圈显示“上游不支持”，不是 0。
...
- T028 persona-complete 回执可用后的 exact 投影合同；本规格不实现该上游能力。
...
- 碎银朋友圈读取、三账号完整 cohort 的生产实现；正式 Handoff/Issue 只定义上游合同，不在本工作区实现。
...
| E002 | SPEC | SPEC-RELATION-010@1.0.0 | 微信 moments 已进 graph；碎银朋友圈当前 unsupported | confirmed |
| E003 | SPEC | SPEC-RELATION-028@1.0.0 | current-allocation partial 与三人设 complete 必须分离 | confirmed |
| E004 | SPEC | SPEC-RELATION-029@1.0.0 | 微信 safe receipt、diff、CAS、legacy 与备份语义 | confirmed |
| E005 | PROTO | prototype/local-vault.js / scripts/suiyin-mcp-client.mjs | 当前可拆 conversationKind、isGroup、excerpt/group_context，但只持久总 messageCount | confirmed |
...
- T010 把碎银朋友圈固定 unsupported；本规格保持该事实，不以用户想看到朋友圈为理由补 0 或 mock。
- T028 当前 persona-complete 仍 upstream-blocked；本规格只定义完整回执的消费方式，不改变阻断结论。
...
| `SPEC-RELATION-028@1.0.0` | 碎银 partial/complete scope 与 per-persona 证据 | 版本或批准态变化 |
...
- **用户**：房总。
- **前置条件**：T028 当前 partial staging 或未来 persona-complete staging。
- **目标**：分别看到单聊/群聊与消息数，并知道范围是否完整。
- **独立价值**：当前上游没补齐时仍可用已有片段核对，不制造“全量完成”。

#### AC-R006-01 — partial 六项拆分

- Given current-allocation partial fixture。
- When 投影来源卡。
- Then 好友、1 对 1 聊天、1 对 1 消息、群聊、群消息均显示 partial 和实际已读值；朋友圈显示 upstream-unsupported/null；卡片明确不是三账号完整范围。
...
- Then 最多报告 preview-validated；T028 complete 与碎银朋友圈仍分别 upstream-blocked / upstream-unsupported。
...
| R006 | 碎银 current-allocation 的好友/聊天/消息只能 partial；朋友圈沿 T010 upstream-unsupported/null | MUST | E002/E003/E005 | OP-030 | AC-R006-01 |
| R007 | 碎银 exact 仅在 T028 persona-complete 全部门成立时允许；本规格不能解除 upstream block | MUST | E003 | OP-030 | AC-R007-01 |
...
| R010 | 无真实数据/E4/E5最多 preview-validated；unsupported/blocked 必须保留 | MUST | E001/E003 | 无 | AC-R010-01 |
...
| partial | 碎银 partial ready | T028 complete 不成立 | partial | 五项 partial，朋友圈上游不支持 | R006/R007 |
| blocked | 读取/证据不足 | roster/cohort/moments prerequisite 缺失 | blocked | 稳定原因，不显示 0 | R002/R007 |
...
| 碎银朋友圈 | 上游未提供朋友圈读取能力 | capability 事实 | 朋友圈 0 条 |
...
- **只模拟、不承诺真实联通的能力**：T028 persona-complete；碎银朋友圈继续 unsupported。
...
- **当前状态**：authorized-for-handoff；房总于 2026-08-20 授权建立微信 exporter 与碎银 MCP 的正式工程 Handoff/Issue。
- **可能涉及的目标仓**：微信 exporter 当前没有独立 GitHub 工程仓，先由 `xfang9528-glitch/relationship-workbench-prototype` 承载外部工程跟踪；T028 complete 与 T031 碎银朋友圈共同涉及 `PetWebOrg/suiyin_mcp`。两者均不在本规格原型实施范围内。
...
- 不为碎银朋友圈预做假数据或假成功。
- 不修改生产仓；工程合同与 Issue 仅按房总 2026-08-20 的明确授权准备/创建，生产实现仍须独立 worktree、Phase、PR 与 review。
...
**审核人**：房总  
**审核日期**：2026-08-20  
**审核备注**：同时授权准备微信 exporter 与碎银 MCP 的正式工程 Handoff/Issue；不授权本工作区跨仓实现生产代码。
