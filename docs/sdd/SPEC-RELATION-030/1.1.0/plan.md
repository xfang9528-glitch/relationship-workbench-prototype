...
> 本 Plan 已随 `SPEC-RELATION-030@1.1.0` 由房总于 2026-08-20 纠正批准。产品/测试实现仍须先签发独立版本化 task contract；生产工程依赖只剩微信 exporter #1。
...
| 事实与推断分离 | PASS | 完整好友、partial、generic unsupported、blocked 严格分态；当前碎银 MCP 不误判 unsupported |
...
| 真实能力承诺 | PASS | 复用现有 MCP；本地 adapter/回执未闭合不冒充 exact，也不称 MCP 缺能力 |
...
- **生产边界**：不进入 wechat-export-toolkit、suiyin_mcp、suiyin-go、Flutter、React 或任何生产仓；微信 exporter 由 #1 独立交付，碎银只在本地 adapter 复用现有 MCP。
...
| Domain | 碎银 staging | `scripts/suiyin-mcp-client.mjs` | 已有 read-only bridge 与 partial 投影 | live schema discovery；映射现有 MCP 的 persona/customer/chat/moments；生成 complete/partial/blocked 回执 |
...
| 读取碎银 | prior receipt 可见 | 显式 MCP read + local mapping | exact/partial 由 T028@1.1.0 完整性回执决定 | adapter/schema/permission/pagination typed reason + prior 保留 | 0 写 / 重试 | R006–R009 |
...
| suiyin-existing-mcp | 现有 MCP complete/partial/失败映射 | fictional tools schema、scopeComplete、pagination、moments fixture | 不调用真实 MCP |
...
| AC-R010 | report gate | 最多 preview-validated | live E4/E5 未验证与具体 local blocked/error 保留 |
...
- 未联通提示：微信完整好友 roster blocked；碎银 adapter/schema/permission/pagination/receipt 未闭合时显示具体 local blocked/error，不得写 MCP upstream capability missing。
...
- 需要展示的状态：微信 exact+blocked、legacy；碎银 exact/partial/local-blocked/error；窄屏折行。
