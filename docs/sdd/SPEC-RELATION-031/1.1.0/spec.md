...
depends_on_specs: "SPEC-RELATION-002@1.0.0, SPEC-RELATION-008@1.0.0, SPEC-RELATION-010@1.0.0, SPEC-RELATION-021@1.0.0, SPEC-RELATION-022@1.0.0, SPEC-RELATION-023@1.0.0, SPEC-RELATION-027@1.0.0, SPEC-RELATION-028@1.1.0, SPEC-RELATION-029@1.0.0"
...
- **完成后变化**：工作台只显示有可信朋友圈来源的真实 moment；微信已有 moment 继续显示，碎银群上下文彻底排除。卡片显示发布者、格式化时间、正文、媒体文字描述和精确来源标签；来源多选按 OR、与文本搜索按 AND。碎银朋友圈复用现有 MCP 已提供的读取能力，由关系助手本地 adapter 映射；未接线、权限、分页或回执失败时具体 fail closed，不用群聊或 mock 补位。
...
- **明确不做**：不展示或迁移 `group_context`、不把聊天 excerpts 当朋友圈、不读取真实数据做测试、不伪造或新增碎银 MCP 接口、不修改生产 MCP/Flutter/Go、不改变 T028@1.1.0/T029 的范围与回执语义。
...
“真实朋友圈”目前只是一层标题，不是严格的数据资格。群聊消息、原始 epoch、内部 stable ID 和粗粒度来源名都可能进入同一渲染链，既容易误解内容来源，也让大数据量下的筛选变慢。关系助手本地 adapter 尚未把现有 MCP 朋友圈读取结果接入可信 moment 流，旧实现还把这个本地缺口误判成上游无能力。

### 2.3 完成后用户能感受到的变化

房总打开工作台时只看到真正的朋友圈内容卡。每张卡都能看懂“谁发布、什么时候、说了什么、是否有媒体文字描述、从我的微信还是哪个碎银官方人设采集”。搜索和来源筛选不会让整页卡顿；碎银读取未闭合时会说明具体本地接线、权限、分页或回执问题，不会误报 MCP 缺能力，也不会拿群聊冒充。
...
| O005 | existing-MCP moments 映射成功或具体 local fail-closed；不产生 upstream capability 需求，T028/T029 回归保持 | adapter contract + regression oracle |
...
- existing-MCP Suiyin moment 的本地 source contract：通过 live schema 选择无副作用读取路径，形成 exact persona attribution、稳定分页和完整性回执；每条 moment 有 stable source-owned identity、发布者、时间、正文/媒体文字描述。
...
- checking、unavailable、ready-empty、filtered-empty、populated、debouncing、stale、classification-saving/error、adapter-blocked、legacy-compatible 和 overflow 状态。
...
- 新增 production MCP 朋友圈能力或在本地 adapter 尚未形成可信回执时展示/推断碎银朋友圈；不以 mock、空数组、群聊或全租户扫描降级。
...
|---|---|---|---|
| 房总 PC 本机 / 当前 Chrome profile | 是 | OPERATIONAL-RELATION-031 | 本机 AES-GCM graph；微信 moment 可用；碎银复用现有 MCP，本地 adapter 待接 |
...
| E003 | SPEC | SPEC-RELATION-010@1.0.0 | 真实微信 moment、50/页、group context 非人物；其中“碎银朋友圈 unsupported”已被本规格纠正 | superseded-in-part |
| E004 | SPEC | SPEC-RELATION-021/027@1.0.0 + SPEC-RELATION-028@1.1.0 | official persona label、collection location、三账号 complete/partial 与 existing-MCP adapter 边界 | confirmed |
...
| E008 | PROTO | `scripts/suiyin-mcp-client.mjs` current staging | 本地 adapter 当前只生成 friend excerpts 与 group_context，尚未映射 moments；只证明 local gap | confirmed-local-gap |
| E009 | USER | 2026-08-20 房总纠正 | 现有 MCP 支持本项目全部读取能力；不得再建只读能力 | confirmed |
...
- T028@1.0.0 把本地 adapter gap 误判为 upstream-blocked；T028@1.1.0 已纠正为复用现有 MCP。T031 不改变 29/26/3、13/13/16528 等旧 partial 事实或 complete receipt 门禁。
...
| `SPEC-RELATION-010@1.0.0` | 真实微信 moment、50/页与 group-context 排除；不继承 Suiyin unsupported 判断 | 同上 |
...
| `SPEC-RELATION-028@1.1.0` | persona cohort/partial/complete 与 existing-MCP adapter | 同上 |
...
### US05 — 现有碎银朋友圈读取不被本地缺口掩盖

#### AC-R009-01 — existing-MCP local adapter gate

- Given 房总确认现有 MCP 已提供所需朋友圈读取能力，但关系助手本地 adapter 尚未映射该 live tool/schema。
- When 打开 feed 或查看碎银来源说明。
- Then 微信 moment 正常显示；碎银朋友圈状态为 `LOCAL_SUIYIN_MOMENTS_ADAPTER_PENDING` 或更具体的 schema/permission/pagination typed error；不宣称 MCP 缺能力，不调用伪接口，不显示 mock/0条成功，不把 `group_context` 补进 feed。

#### AC-R010-01 — 现有 MCP 的合规接入

- Given fictional live MCP schema/response 可由本地 adapter 映射为 exact persona provenance、stable pagination/completeness receipt 和安全 moment shape。
- When T031@1.1.0 由新本地 task contract 授权并通过 adapter tests。
- Then 允许 mapped Suiyin moments 进入 staging/feed；不修改生产 MCP，不自动读取，不以 fictional tests 宣称真实数据或 live E4/E5 已验证。

### US06 — 保留既有来源与回执事实

#### AC-R011-01 — T028/T029 回归

- Given current-allocation partial、persona-complete receipt gate、微信 reimport receipt 与 legacy source。
- When 实现或使用 feed。
...
| R009 | 必须复用现有 MCP moments 读取；本地 adapter 未接、schema/permission/pagination/receipt 失败必须具体 fail closed，禁止称 upstream missing 或以 group_context/mock/tenant scan 兜底 | MUST | E001/E003/E008/E009 | OP-031 | AC-R009-01 |
| R010 | Suiyin moment 只在本地 adapter 证明 exact record provenance、stable pagination/completeness 和安全 shape 后接入；不新增生产 MCP 能力 | MUST | E001/E004/E008/E009 | OP-031 | AC-R010-01 |
| R011 | T028 partial/complete receipt 与 T029 diff/receipt/time/legacy 语义不得被 feed 改变 | MUST | E004/E006 | OP-031 | AC-R011-01 |

### 6.1 不变量

- INV001：朋友圈 feed 中永远没有 `group_context` 或 chat excerpt。
- INV002：本地尚未接入或一次读取失败不等于 MCP 无能力、不等于 0 条，也不等于当前分配聊天已完整读取。
...
| Suiyin moments | existing-capability / local-adapter-pending | bzds current integration | E008/E009 | adapter 合同升级后重审 |
...
| Suiyin adapter pending/error | 查看碎银状态 | local mapping/schema/permission/page/receipt 未闭合 | adapter-blocked | 明确具体本地/运行时状态，不写接口无能力 | R009 |
...
| adapter提示 | blocked | N/A | 可读文本 | N/A | Suiyin映射未闭合 | N/A | N/A | typed reason | 长说明换行 | 不跨生产仓 |
...
| Suiyin adapter blocked | 现有碎银读取尚未形成可验证的朋友圈回执；群聊不会出现在这里 | 诚实本地/运行时状态 | 当前碎银接口未提供能力 / 碎银朋友圈0条 / 已读取 |
...
|---|---|---|---|---|
| R001–R011 | 本机Sources feed | 不适用 | 不适用 | 仅房总/bzds；Suiyin existing capability，本地 adapter 待接 |

## 10. 原型实现契约

- **目标原型仓**：`E:/AI 项目/关系维护助手`。
- **目标文件 / 路由**：未来批准后可修改 `scripts/suiyin-mcp-client.mjs`、`prototype/local-vault.js`、`prototype/index.html` 与 focused fictional tests；路由保持 `#/sources`。
...
- **必须模拟的状态**：mixed moment/group-context、legacy kindless WeChat、source conflict、invalid time、10k/20k、debounce stale、generation stale、adapter/schema/permission/pagination blocked。
- **关键 mock 数据**：全部code-authored fictional names/body/IDs/times；privacy canary不可对应真人。
- **只模拟、不读取私人数据**：使用 code-authored fictional live-tool schemas/responses 验证现有 MCP adapter；不创建生产 stub，不声称 live 联通。
...
| R009/R010 | existing-MCP adapter + Suiyin status panel | 显式读取/打开Sources | AC-R009-01/AC-R010-01 |
| R011 | existing source flows | partial/reimport/reopen | AC-R011-01 |

### 10.2 工程交付准备

- **当前状态**：corrected-local-handoff；错误的碎银 MCP Handoff/#19 已撤回，不创建新的生产 Issue。
- **可能涉及的目标仓**：仅本私有关系助手原型/adapter；现有 MCP 作为读取来源，不是修改目标。
- **优先自动化的验收**：AC-R001-01–AC-R011-01，全部使用fictional fixtures。
- **必须人工判断的验收**：内容流阅读密度、长正文折叠和focus体验；需要isolated public preview，不读取真实数据。
- **Handoff / Test Contract**：见同目录 `issue-handoff.md` 与 `test-contract.md`；`actual_issue_creation:false`，本地实现须另签 task contract，不建 MCP Issue。

## 11. 非目标与约束

- 不把“统一”理解为把聊天与朋友圈混在一条流。
- 不为现有 MCP 写替代 stub、假成功、空数组或 mock 生产路径。
- 不改变source cards六项统计、People collection location或关系建议。
- 不运行真实微信导出、真实IDB、真实MCP、private DOM、network/model/upload/send测试。
- 不跨生产仓；旧 MCP Issue/Handoff 已撤回，只允许关系助手本地 adapter 工作。
...
**审核结论**：approved；只展示 trusted moment，永久排除 `group_context`/聊天 excerpt；来源按记录自身 provenance；碎银复用现有 MCP，由本地 adapter 映射并具体 fail closed。
**审核人**：房总
**审核日期**：2026-08-20
**审核备注**：2026-08-20 纠正：撤回碎银 MCP 新能力 Handoff/#19；“只读”仅为关系助手调用边界，现有 MCP 已覆盖所需读取。

## 14. 变更历史

| 版本 | 日期 | 作者 | 变化 | 原因 |
|---|---|---|---|---|
| 1.1.0 | 2026-08-20 | Codex E1/E2 | 撤销 upstream capability 缺失判断；R009/R010 改为 existing-MCP 本地 adapter；删除 MCP 工程 Slice | 房总纠正确认现有 MCP 已支持全部读取能力 |
