...
| 项 | 当前事实 |
|---|---|
| SPEC | `SPEC-RELATION-031@1.1.0`，approved |
| Plan | approved |
| Operational Profile delta | approved；继承2026-08-13本机hosted-stateful总例外 |
| Source Contract | approved；existing MCP capability available，local adapter pending |
...
- [x] 房总审核并明确纠正批准`SPEC-RELATION-031@1.1.0`、Plan、Operational Profile增量与Source Contract。
...
- [ ] 微信 ready 与碎银 exact/partial/adapter-blocked 可同时呈现，不用“0条/已读取”冒充映射完成。
...
## T031-06 — Existing MCP Adapter Wiring and Fail-Closed

- [ ] 用 fictional `tools/list`/input schema/response 把现有 MCP moments 读取映射为 exact persona provenance、stable pagination/snapshot 与 completeness receipt；不编造工具名/参数。
- [ ] adapter/schema/permission/pagination/receipt 未闭合时返回 `LOCAL_SUIYIN_MOMENTS_ADAPTER_PENDING` 或更具体 typed error；证明 0 mock/空成功、0 group fallback、0 tenant scan。
- [ ] 本地 adapter 实现必须先签发 task contract；不得新增生产 MCP API 或替代 stub。
...
- [ ] 保持T028 current-allocation partial、29/26/3、13 friend/13 group/16528 messages、persona-complete receipt gate与official registry语义。
...
- 现有 MCP 能力是已确认前提；本地 adapter/权限/分页/回执未闭合时诚实 local-blocked，禁止为了“完成”而造数据、复用群聊或转建 MCP 能力 Issue。
