...
- 本Profile是`SPEC-RELATION-031@1.1.0`的已批准能力增量合同。
...
- 修改MCP/Flutter/React/Go生产仓，或把本地 adapter 测试结果冒充真实联通。
...
- 房总确认现有 MCP 已具备本项目所需全部读取能力；关系助手只在房总显式点击后通过现有 loopback bridge 调用 allowlisted 读取工具。
- 本地 adapter 负责 live schema discovery、exact persona provenance、稳定分页和 completeness receipt；未闭合时返回具体 local typed 状态，不得称 MCP 缺能力。
- 不创建 production adapter stub、不返回 mock/空成功、不调用聊天 history 后转成朋友圈、不从 `group_context` 降级；实现仍须新 task contract。
...
- public错误使用稳定文案；本地接线、权限、分页或回执未闭合不显示“0条/已读取”，也不显示“上游未提供能力”。
...
- [x] 微信可用、碎银 existing-MCP capability available / local adapter pending 的事实准确。
- [x] 网络、日志、真实数据与生产仓禁区准确。
- [x] 可以由独立task contract授权实施。

**审核结论**：approved；本机增量不扩展网络或共享边界，碎银 moments 复用现有 MCP，仅本地 adapter 待接。
**审核人**：房总
**审核日期**：2026-08-20
