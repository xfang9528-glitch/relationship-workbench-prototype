...
version: 1.0.0
status: approved
source_spec: SPEC-RELATION-031@1.0.0
...
| 碎银官方人设 | production MCP的persona-filtered read-only moments | unavailable | `UPSTREAM_SUIYIN_MOMENTS_UNAVAILABLE` |
...
- exact source evidence：future moment row本身携带可验证的source-owned persona attribution，能与T021/T028的encrypted official registry精确闭合。
- public label：`碎银 · {官方人设名}`。
- 若attribution缺失、冲突、persona不在current registry或response applied filter不闭合，整条/整批按future adapter contract fail closed；不得猜“1/2/3号”。

人物可能在多个采集位置出现，但人物的全局collection-location badge不能决定某条moment的来源。来源只取该moment自身provenance。

## 5. Future Suiyin Moments Capability Requirements

当前不定义或调用生产adapter。若未来生产MCP提供等价只读能力，至少必须满足：
...
等价字段名可以在未来评审中映射，但以下能力不可省略：

- persona filter必须由upstream exact应用，不能先全租户扫描再由prototype猜分配；
...
能力尚未满足时，唯一合法结果是`UPSTREAM_SUIYIN_MOMENTS_UNAVAILABLE`。禁止adapter stub、mock、空数组成功、group_context fallback或环境切换工具。
...
- T028 current-allocation partial、allocation missing、friend/group/message counts、per-persona official labels与persona-complete blocker保持不变。
...
- production MCP新增或修改moments方法/字段；
...
当前审核结论：approved。  
当前批准人：房总，2026-08-20。  
当前实施授权：本机原型仍须独立 task contract；生产 MCP 只授权正式 Handoff/Issue，不授权本工作区实现。
