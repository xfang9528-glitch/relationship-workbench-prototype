...
> 本 Plan 已随 `SPEC-RELATION-031@1.0.0` 由房总于 2026-08-20 批准。它只描述可逆原型实现路径；签发独立版本化 task contract 前，不授权修改产品/测试，生产能力只走正式 Handoff/Issue。
...
| 状态完整 | PASS | checking、empty、filtered-empty、stale、saving/error、upstream-blocked均入合同 |
| 平台与租户边界 | PASS | 仅房总本机browser profile + loopback origin |
| 真实能力承诺 | PASS | 微信已有moment可展示；碎银moments在上游能力到位前明确阻断 |
...
- **网络与生产边界**：当前实现不得新增网络调用或修改`E:/dev`生产仓。碎银moments只有在未来persona-filtered read-only MCP真实部署、source contract重审和新合同签发后才可进入实施范围。
...
| Upstream | T028 cohort/blocker | Suiyin read boundary | 已有current-allocation partial与persona-complete blocker | 仅复用blocked语义，不拿聊天当moment |
...
`scripts/suiyin-mcp-client.mjs`、任意production MCP/Flutter/React/Go文件不在候选写集合。未来上游能力接入必须另开评审与合同，不能在T031当前合同中偷渡。
...
2. “真实朋友圈内容流”是独立feed shell，包含：
   ...
   - 文本、身份、分类条件；
   - 最多50张内容卡与分页；
   - 微信ready与碎银upstream-blocked可同时真实呈现。
...
| 查看碎银来源 | blocked说明 | 无远程动作 | 仍可看微信moment | fixed upstream reason | 等生产能力另行评审 | R009/R010 |
...
| regression | T028 partial/blocker、T029 receipt/legacy | approved exact shapes | 不读真实数据 |
...
| AC-R009-01/AC-R010-01 | current capability gate | fixed blocker、0 mock/tenant scan/MCP调用 | capability oracle |
...
- **未来上游能力与本合同不一致**：保持`UPSTREAM_SUIYIN_MOMENTS_UNAVAILABLE`，重审source contract而非兼容猜测。
...
- **重点画面**：微信populated + 碎银blocked并存；多来源OR+文本AND；filtered-empty；长正文/媒体描述；invalid time；50卡分页。
